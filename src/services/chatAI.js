 ...
 * Store keys in .env.local (e.g. NEXT_PUBLIC_HUGGINGFACE_API_KEY) - never commit real keys.
 */
...
export async function getAIResponse(userMessage, conversationHistory, portfolioData) {
  ...
  const hfKey = process.env.NEXT_PUBLIC_HUGGINGFACE_API_KEY?.trim()
...
export function hasAIKey() {
  const api = (process.env.NEXT_PUBLIC_CHAT_API_URL || '').trim()
  const h = (process.env.NEXT_PUBLIC_HUGGINGFACE_API_KEY || '').trim()
  const g = (process.env.NEXT_PUBLIC_GEMINI_API_KEY || '').trim()
  const r = (process.env.NEXT_PUBLIC_GROQ_API_KEY || '').trim()
  const o = (process.env.NEXT_PUBLIC_OPENAI_API_KEY || '').trim()
  return !!(api || h || g || r || o)
}
